...
async def Gemini_API_async(model, prompt, api_key=GOOGLE_API_KEY):
    max_retries = 5
    for i in range(max_retries):
        try:
            genai.configure(api_key=api_key)
            ...
            print(f'************* Gemini Async Retrying ({i+1}) *************')
            logging.error(f"Gemini Async Error: {e}")
            if i < max_retries - 1:
...
def ChatGPT_API(model, prompt, api_key=CHATGPT_API_KEY, chat_history=None):
    if "gemini" in model.lower() or (not api_key and GOOGLE_API_KEY):
        ...
        if "gpt" in model.lower():
            model = os.getenv("PYTHON_LLM_MODEL", "gpt-4o")
